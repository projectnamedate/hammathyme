...
- **Wordmark direction:** canonical recipe is a hybrid. Use the Motion Reel `01 - cold open` per-letter Outfit Black construction for the `hammer` lettering, preserve its approved visual rhythm, and use the current website period placement for the controlled circular mark. Do not replace the lettering with a native `.kw` text run.
- **Characters at launch:** Kira is the canonical autonomous-character source; the public agents case study is live at `/work/agents/kira`.
...
- Reel signature size: `font-size: 228px`, `line-height: 0.84`, `letter-spacing: 0`. Site surfaces scale the period from the website baseline placement below.
...
- Letter gaps after `h a m m e r`: `-0.018em`, `-0.055em`, `-0.045em`, `-0.020em`, `-0.022em`, `0.007em`. These preserve the approved doc-flow lettering rhythm with a slightly looser `m-e` pair; do not replace them with mathematically equal ink-box gaps because that creates visible crowding.
- Period: render as a controlled cinnamon circle, not the font period glyph. For the website and UI, use the approved baseline placement from the current site: `width: 0.171em`, `height: 0.171em`, `margin-left: -0.009em`, `transform: translateY(0.012em)`, and baseline alignment. Reel compositions may keep their absolute dot wrapper when needed for scene geometry, but the website period placement is canonical for site surfaces.
...
1. **Protected brand mark.** Only `hammer.` gets the canonical brand mark. Render `hammer` with the cold-open per-letter Outfit Black recipe and render the period with the approved website baseline placement, never the font period glyph. The dot is always `--cinnamon`, baseline-aligned, and proportioned from the canonical wordmark recipe above.
